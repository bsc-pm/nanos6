...
CUfunction CUDARuntimeLoader::loadFunction(const char* str)
{
    ...
    auto it = _function_cache[currentDevice].find(str);
    if(it != _function_cache[currentDevice].end()) return it->second;

    CUfunction fnc;
    for(CUmodule mod : _per_gpu_modules[currentDevice])
    {
        if(cuModuleGetFunction(&fnc, mod, str) == CUDA_SUCCESS) 
        {
            _function_cache[currentDevice][str] = fnc;
            return fnc;
        }
    }
...
void CUDARuntimeLoader::getSectionBinaryModules(std::vector<name_data>& cu_objs)
{

    const char* self_exe = "/proc/self/exe";
    
    struct stat st;
    if (stat(self_exe, &st) != 0) {
        FatalErrorHandler::fail("Cannot stat file: ", self_exe, "\n");
    }
    ...
    const char *const sh_strtab_exe = exe + sh_strtab->sh_offset;


    for (Elf32_Half i = 0; i < shnum; ++i) 
    {
        if(std::string(sh_strtab_exe + shdr[i].sh_name) == ".nv_fatbin")
        {
            nv_fatbin_desc* desc = (nv_fatbin_desc*) &exe[shdr[i].sh_offset];
            
            while(desc->magic == NV_FATBIN_DESC_MAGIC)
            {
                cu_objs.emplace_back(CUDA_BINARY_STRING, std::string((char*)desc, desc->size));

                desc = (nv_fatbin_desc*) ((char*)desc + desc->size + 16); // 16 = 4 magic + 4 version + 8 size
                desc = (nv_fatbin_desc*) ((((uintptr_t) desc) + 15) & ~15); //align 16 bytes
            }
            break;
        }
    }
...
{
    _function_cache.resize(contexts.size());
    _per_gpu_modules.resize(contexts.size());
    //load kernels from the kernels folder
    std::string kernels{ConfigVariable<std::string>("devices.cuda.kernels_folder")};

    std::vector<name_data> cu_objs;
    std::vector<name_data> cu_srcs;
    std::vector<name_data> cu_headers;


    for(auto const& dir_entry : getAllCudaUserBinariesPath(kernels))
    {

        if(dir_entry.size()>2 && dir_entry.substr(dir_entry.size()-2) == ".c") continue;
        if(dir_entry.size()>4 && dir_entry.substr(dir_entry.size()-4) == ".cpp") continue;

        if((dir_entry.size()>4 && dir_entry.substr(dir_entry.size()-4) == ".hpp") ||(dir_entry.size()>2 && dir_entry.substr(dir_entry.size()-2) == ".h"))
            cu_headers.emplace_back(dir_entry, getFileContent(dir_entry));
        else if(dir_entry.size()>3 && dir_entry.substr(dir_entry.size()-3) == ".cu" )
            cu_srcs.emplace_back(dir_entry, getFileContent(dir_entry));
        else 
            cu_objs.emplace_back(dir_entry, getFileContent(dir_entry));
    }

    //When this is emplaced after the folder elements, the kernels in the folder are overriding implemented functions
    getSectionBinaryModules(cu_objs);

    bool warning_enabled = ConfigVariable<bool>("devices.cuda.warning_on_incompatible_binary");
    for(size_t i = 0; i < contexts.size(); ++i)
    {
        cuCtxSetCurrent(contexts[i]);

        std::vector<std::string> cu_src_ptx_vec = compileSourcesIntoPtxForContext(cu_srcs, cu_headers, i);
        for(std::string& cu_src_ptx : cu_src_ptx_vec)
        {
            CUmodule tmp;
            if(cuModuleLoadData(&tmp, &cu_src_ptx[0]) == CUDA_SUCCESS) _per_gpu_modules[i].push_back(tmp);
            else FatalErrorHandler::warn("Could not load compiled ptx");
        }

        for(name_data& cu_obj : cu_objs)
        {
            CUmodule tmp;
            if(cuModuleLoadData(&tmp, &cu_obj.data[0]) == CUDA_SUCCESS) _per_gpu_modules[i].push_back(tmp);
            else 
            {
                if(cu_obj.filename == CUDA_BINARY_STRING)
                {
                    if(!warning_enabled) continue;
                ...
                else FatalErrorHandler::warn("Could not load ptx file: ", cu_obj.filename);
            }
