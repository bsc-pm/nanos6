...
{
	FatalErrorHandler::printIf(_verbose,
	...
	for (size_t i = 0; i < _enabledPAPIEventCodes.size(); ++i) {
		...
		if (_verbose) {
			...
			int retName = PAPI_event_code_to_name(eventCode, codeName);
			if (retName != PAPI_OK) {
				FatalErrorHandler::fail(
					retName, " when converting from PAPI code to PAPI event name - ",
					PAPI_strerror(retName)
				);
			}

			if (ret != PAPI_OK) {
				FatalErrorHandler::print("  - Enabling ", codeName, ": FAIL");
			} else {
				FatalErrorHandler::print("  - Enabling ", codeName, ": OK");
			}
		}
		...
		if (ret != PAPI_OK) {
			FatalErrorHandler::fail(
				"Cannot simultaneously enable all the requested PAPI events due to incompatibilities"
			);
		}
	}
	...
	if (ret != PAPI_OK) {
		FatalErrorHandler::fail(
			ret, " when clearing the main thread's PAPI eventSet - ",
			PAPI_strerror(ret)
		);
	}
...
PAPIHardwareCounters::PAPIHardwareCounters(
	...
	*/
	...
	FatalErrorHandler::printIf(_verbose,
		"------------------------------------------------\n",
		"- Testing requested PAPI events availabilities"
	);
	...
	while (it != enabledEvents.end()) {
		...
		if (id >= HWCounters::HWC_PAPI_MIN_EVENT && id <= HWCounters::HWC_PAPI_MAX_EVENT) {
			...
			if (_verbose) {
				if (ret != PAPI_OK) {
					FatalErrorHandler::print("  - ", HWCounters::counterDescriptions[id], ": FAIL");
				} else {
					FatalErrorHandler::print("  - ", HWCounters::counterDescriptions[id], ": OK");
				}
			}

			if (ret != PAPI_OK) {
				FatalErrorHandler::warn(ret, " ", HWCounters::counterDescriptions[id],
					" event unknown in this version of PAPI, skipping it - ", PAPI_strerror(ret)
				);
	...
	FatalErrorHandler::printIf(_verbose,
		...
		"- Number of PAPI events enabled: ", _numEnabledCounters, "\n",
		"------------------------------------------------"
	);
}
...
void PAPIHardwareCounters::threadInitialized(ThreadHardwareCountersInterface *threadCounters)
{
	if (_enabled) {
		...
		*/
		...
		ret = PAPI_add_events(eventSet, _enabledPAPIEventCodes.data(), _enabledPAPIEventCodes.size());
		if (ret != PAPI_OK) {
			FatalErrorHandler::fail(
