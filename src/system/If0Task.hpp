...
namespace If0Task {
	inline void waitForIf0Task(WorkerThread *currentThread, Task *currentTask, Task *if0Task, ComputePlace *hardwarePlace)
	{
	...
	inline void executeInline(
		...
		__attribute__((unused)) ComputePlace *hardwarePlace
	) {
		assert(currentThread != nullptr);
		assert(currentTask != nullptr);
		assert(if0Task != nullptr);
