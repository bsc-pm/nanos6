...
namespace Instrument {
	...
	struct InstrumentationContext {
		...
		InstrumentationContext(
			task_id_t const &taskId,
			compute_place_id_t const &computePlaceId,
			thread_id_t const &threadId
		) :
			_taskId(taskId),
			_computePlaceId(computePlaceId),
			_threadId(threadId),
			_externalThreadName(nullptr)
		{
		}
		...
		InstrumentationContext(InstrumentationContext const &other) :
			...
			_computePlaceId(other._computePlaceId),
		...
		InstrumentationContext& operator=(const InstrumentationContext& other)
		{
			_taskId = other._taskId;
			_computePlaceId = other._computePlaceId;
			_threadId = other._threadId;
